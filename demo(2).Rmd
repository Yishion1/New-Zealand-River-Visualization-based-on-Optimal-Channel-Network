...
# 将sf对象转换为WGS84坐标系统 (EPSG:4326)
riverLines_water_quality_AN_wgs84 <- st_transform(riverLines_water_quality_AN, crs =st_crs(rivers_sf ))
riverLines_water_quality_NN_wgs84 <- st_transform(riverLines_water_quality_NN, crs =st_crs(rivers_sf ))
}
...
server <- function(input, output, session) {
  ...
  reactiveRiverData <- reactive({
    ...
    # 根据用户选择连接不同的水质数据集
    if (input$pollutant == "AN") {
      water_quality_data <- st_transform(riverLines_water_quality_AN, crs = st_crs(rivers_sf))
    } else if (input$pollutant == "NN") {
      water_quality_data <- st_transform(riverLines_water_quality_NN, crs = st_crs(rivers_sf))
    }
    
    # 过滤数据以匹配用户选择的测量方法
    filtered_data <- water_quality_data %>% 
      filter(mesrmnt == input$measurement)
    rivers_sf <- rivers_sf %>% st_join(filtered_data, by = "nzsegment")
    ...
    rivers_sf %>% distinct(OBJECTID_1, .keep_all = TRUE)
  })
