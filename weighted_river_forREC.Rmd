...
water_quality_influences <- final_segments_waterquality$value

influence_results <- calculateInfluenceDynamically_REC(upstreamInfluenceWeight=0.1, final_river_segments = final_segments_waterquality,connectivity = 20,influences = water_quality_influences)
...
for (weight in seq(0.1, 1, by=0.1)) {
  influence_results <- calculateInfluenceDynamically_REC(connectivity=10, upstreamInfluenceWeight=weight, final_river_segments = final_segments_waterquality,influences = water_quality_influences)
  normalized_influences <- (influence_results - min(influence_results)) / (max(influence_results) - min(influence_results))
  # 将当前权重下的影响力结果添加到数据框
...
for (conn in seq(1,20,6)) {
  influence_results <- calculateInfluenceDynamically_REC(connectivity=conn, upstreamInfluenceWeight=0.7, final_river_segments)
  normalized_influences <- (influence_results - min(influence_results)) / (max(influence_results) - min(influence_results))
