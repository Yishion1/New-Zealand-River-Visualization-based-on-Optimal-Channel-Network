...
```{r}
calculateInfluenceDynamically_REC <- function(final_river_segments, upstreamInfluenceWeight, connectivity) {
...
influence_results <- calculateInfluenceDynamically_REC(upstreamInfluenceWeight=0.4, final_river_segments,connectivity = 20)
...
for (weight in seq(0.1, 1, by=0.1)) {
  influence_results <- calculateInfluenceDynamically_REC(connectivity=10, upstreamInfluenceWeight=weight, final_river_segments)
  normalized_influences <- (influence_results - min(influence_results)) / (max(influence_results) - min(influence_results))
  # 将当前权重下的影响力结果添加到数据框
