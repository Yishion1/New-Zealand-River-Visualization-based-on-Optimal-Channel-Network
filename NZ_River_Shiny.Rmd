...
```
```{r}
connected_river_segments <- riverLines_water_quality_AN_wgs84[riverLines_water_quality_AN_wgs84$HydroID %in% Kahiaka_connected_ID, ]
...
# 创建地图
leaflet_map <- leaflet(data = connected_river_segments) %>%
  addProviderTiles(providers$OpenStreetMap) %>%
  addPolylines(color = "#2A1B81", weight = ~6 / StreamOrde, opacity = 1) %>%
